...
export default function MoviesIndex({ movies, pagination, filters = {} }: MoviesIndexProps) {
    ...
    function handleSort(newSortBy: string) {
        ...
        router.get('/movies', { sortBy: newSortBy, ascOrDesc: newOrder, page: 1, perPage }, { preserveScroll: false });
    }

    function handleOrder(order: string) {
        router.get('/movies', { sortBy, ascOrDesc: order, page: 1, perPage }, { preserveScroll: false });
    }

    function handlePerPage(newPerPage: string) {
        router.get('/movies', { sortBy, ascOrDesc, page: 1, perPage: newPerPage }, { preserveScroll: false });
    }
    ...
    return (
        <AppLayout title="Movies">
            <div className="space-y-6">
                ...
                {pagination?.total > 0 && (
                    <p className="text-gray-400 text-sm">{pagination.total.toLocaleString()} movies found</p>
                )}
                        ...
                        {movies.map((movie) => (
                            <Link key={movie.id} href={`/movies/${movie.id}`} className="group bg-gray-900 rounded-xl overflow-hidden border border-gray-700 hover:border-indigo-500 transition-colors">
                                <div className="aspect-[2/3] bg-gray-800 overflow-hidden">
                                    <img
                                        src={movie.photoSrc ? `/images/movies/${movie.photoSrc}` : '/images/placeholder.jpg'}
                                        alt={movie.title}
                                ...
                                <div className="p-3">
                                    ...
                                    <div className="flex items-center justify-between mt-1">
                                        ...
                                        {movie.ratingImdb && (
                                            <span className="text-xs text-yellow-400 font-medium">{movie.ratingImdb.toFixed(1)}</span>
                                        )}
                                    </div>
                                </div>
                            </Link>
