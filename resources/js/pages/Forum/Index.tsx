                ...
                <div className="space-y-3">
                    {categories.map((cat) => (
                        <Link key={cat.id} href={`/forum/categories/${cat.id}`}
                            className="flex items-center gap-4 bg-card border border-border hover:border-primary rounded-xl p-5 transition-colors group">
                            ...
                            {cat.topicCount !== undefined && (
                                <span className="text-sm text-muted-foreground">{cat.topicCount} topics</span>
                            )}
                        </Link>
                    ))}
                </div>
                {categories.length === 0 && (
                    <p className="text-muted-foreground text-center py-10">No forum categories yet.</p>
                )}
            </div>
        </AppLayout>
    );
