...
export class SerieService {
    ...
    async findByGenre(genreId: number, userId?: number, page: number = 1, perPage: number = 12): Promise<SerieListResponseDto> {
        const skip = (page - 1) * perPage;

        const seriesByGenre = await this.prisma.serieGenre.findMany({
            where: { genreId },
            select: { serieId: true },
            skip,
            take: perPage,
        ...
        if (!seriesByGenre.length) {
            return SerieMapper.toListResponseDto({ series: [], count: 0 });
        }

        const serieIds = seriesByGenre.map((sg) => sg.serieId);
        const series = await this.prisma.serie.findMany({
        ...
        const totalCount = await this.prisma.serieGenre.count({ where: { genreId } });

        return SerieMapper.toListResponseDto({ series: seriesWithDetails, count: totalCount });
    }

    async findByCast(actorId: number, userId?: number, page: number = 1, perPage: number = 12): Promise<SerieListResponseDto> {
        const skip = (page - 1) * perPage;

        const seriesByCast = await this.prisma.castSerie.findMany({
            where: { actorId },
            select: { serieId: true },
            skip,
            take: perPage,
        ...
        if (!seriesByCast.length) {
            return SerieMapper.toListResponseDto({ series: [], count: 0 });
        }

        const serieIds = seriesByCast.map((cs) => cs.serieId);
        const series = await this.prisma.serie.findMany({
            where: { id: { in: serieIds } },
        });
        ...
        const totalCount = await this.prisma.castSerie.count({ where: { actorId } });

        return SerieMapper.toListResponseDto({ series: seriesWithDetails, count: totalCount });
    }

    async findByCrew(crewId: number, userId?: number, page: number = 1, perPage: number = 12): Promise<SerieListResponseDto> {
        const skip = (page - 1) * perPage;

        const seriesByCrew = await this.prisma.crewSerie.findMany({
            where: { crewId },
            select: { serieId: true },
            skip,
            take: perPage,
        ...
        if (!seriesByCrew.length) {
            return SerieMapper.toListResponseDto({ series: [], count: 0 });
        }

        const serieIds = seriesByCrew.map((cs) => cs.serieId);
        const series = await this.prisma.serie.findMany({
            where: { id: { in: serieIds } },
        });
        ...
        const totalCount = await this.prisma.crewSerie.count({ where: { crewId } });
